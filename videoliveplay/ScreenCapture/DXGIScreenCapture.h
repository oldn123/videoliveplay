...
class DXGIScreenCapture : public IFrameCapture
{
public:
	DXGIScreenCapture();
	...
	int init(int displayIndex = 0) override;
	int exit() override;
	int start() override;
	int stop() override;
	...
	inline int getWidth() override { return m_dxgiDesc.ModeDesc.Width; }
	inline int getHeight() override { return m_dxgiDesc.ModeDesc.Height; }

	int captureFrame(std::shared_ptr<uint8_t>& bgraPtr, uint32_t& size) override;
