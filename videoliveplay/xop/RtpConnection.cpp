...
void RtpConnection::setFrameType(uint8_t frameType)
{
    ...
    if (!_JustAudioMode)
    {
        if (_AudioFrameCounter >= 0 && _frameType == AUDIO_FRAME)
        {
            _AudioFrameCounter++;
            if (_AudioFrameCounter >= 60)
            {
                _JustAudioMode = true;
                std::cout << "audio mode" << std::endl;
            }
        }
        else
        {
            if (_AudioFrameCounter >= 0)
            {
                _JustAudioMode = false;
                _AudioFrameCounter = -1;
                std::cout << "video && audio mode" << std::endl;
            }
        }
    }
...
{
    if((_mediaChannelInfo[channelId].isPlay || _mediaChannelInfo[channelId].isRecord) && (_hasIDRFrame || (_JustAudioMode && _frameType == AUDIO_FRAME)))
    {
