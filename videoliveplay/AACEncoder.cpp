...
bool AACEncoder::init(struct AudioConfig ac)
{
	...
	_aCodecCtx->profile =/* FF_PROFILE_AAC_MAIN; */FF_PROFILE_AAC_LOW;
	_aCodecCtx->strict_std_compliance = FF_COMPLIANCE_EXPERIMENTAL;
	...
	SPcm2AacParam sp = { _audioConfig.samplerate, _audioConfig.channels, _audioConfig.bitPreSample };
    m_faacEnc.Initilize(sp);
#endif
...
AVPacket* AACEncoder::encodeAudio(const uint8_t *pcm, int samples, std::vector<uint8_t>* pOutData)
{
#ifdef MUXER_PCMTOAAC_H
	int nol = 1024;
    if (nol > 0 && pOutData)
    {
        pOutData->resize(nol);
		nol = m_faacEnc.Encode((UInt8*)pcm, samples, pOutData->data(), nol);
        if (nol < 1)
        {
            pOutData->clear();
        }
        ...
        {
            pOutData->erase(pOutData->begin() + nol, pOutData->end());
        }
        return nullptr;
    }
#endif

	return encodeAudio2(pcm, samples);
}

AVPacket* AACEncoder::encodeAudio2(const uint8_t *pcm, int samples)
{
