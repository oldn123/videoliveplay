...
bool RtcVideoSink::setVideoWindow(void * wnd)
{
...
bool RtcVideoSink::renderYuv(int width, int height, const uint8_t* Yplane, int Ypitch,
    ...
    if (m_startRender)
        m_sdkRenderer->loadYuv(width, height, Yplane, Ypitch, Uplane, Upitch, Vplane, Vpitch);
    return true;
}
