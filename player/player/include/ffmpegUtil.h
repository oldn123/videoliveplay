...
namespace ffmpegUtil {
    ...
    struct ffUtils {
        static void initCodecContext(AVFormatContext* f, int streamIndex, AVCodecContext** ctx) {
            ...
            AVCodec* codec = avcodec_find_decoder(f->streams[streamIndex]->codecpar->codec_id); //根据指定的AVCodecID查找注册的解码器

            if (codec == nullptr) {
                std::string errorMsg = "Could not find codec: ";
                errorMsg += (*ctx)->codec_id;
                cout << errorMsg << std::endl;
                throw std::runtime_error(errorMsg);
            }

            (*ctx) = avcodec_alloc_context3(codec); //为AVCodecContext分配内存
            auto codecCtx = *ctx;

            if (avcodec_parameters_to_context(codecCtx, f->streams[streamIndex]->codecpar) != 0) {
                std::string errorMsg = "Could not copy codec context: ";
                errorMsg += codec->name;
                cout << errorMsg << std::endl;
                throw std::runtime_error(errorMsg);
            }

            if (avcodec_open2(codecCtx, codec, nullptr) < 0) { //打开解码器
...
//                throw std::runtime_error(errorMsg);
    ...
    public:
        ...
        PacketGrabber(const std::string& uri) : inputUrl(uri) {
            formatCtx = avformat_alloc_context();

            if (avformat_open_input(&formatCtx, inputUrl.c_str(), NULL, NULL) != 0) {
                std::string errorMsg = "Can not open input file:";
                errorMsg += inputUrl;
                cout << errorMsg << endl;
                throw std::runtime_error(errorMsg);
            }
            ...
            if (avformat_find_stream_info(formatCtx, NULL) < 0) {
                std::string errorMsg = "Can not find stream information in input file:";
                errorMsg += inputUrl;
                cout << errorMsg << endl;
                throw std::runtime_error(errorMsg);
            }
